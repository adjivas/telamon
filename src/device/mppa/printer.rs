...
pub fn print(fun: &Function, time: bool, out: &mut io::Write) -> io::Result<()> {
    lazy_static! {
        static ref ID: atomic::AtomicUsize = atomic::AtomicUsize::new(0);
    }
    ...
    let kernel_args = fun.params
        .iter()
        .map(|p| format!("{} {}", type_name(&p.t), p.name))
        .chain({
            if time {
                Some("uint64_t* __time_ptr".to_string())
            } else {
                None
            }
        })
...
) -> io::Result<()>
{
    let arg_names = signature
        .params
        .iter()
        .format_with("", |p, f| f(&format_args!("{}, ", p.name)));
    let cl_arg_defs = signature
        .params
        .iter()
        .format_with("", |p, f| {
...
) -> fmt::Result
{
    match *cfg {
        ...
        Cfg::Barrier => writeln!(out, "  assert(!pthread_barrier_wait(&barrier));"),
