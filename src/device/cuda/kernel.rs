...
use device::cuda::{api, ArrayArg, Context, Gpu, JITDaemon, PerfCounterSet};
use ir;
...
impl<'a, 'b> Kernel<'a, 'b> {
    ...
    pub fn evaluate(&self, args: &Context) -> Result<u64, ()> {
        ...
        self.gen_args(args)
            .execute(&cuda_kernel, self.executor)
    }
...
impl<'a> ThunkArgs<'a> {
    ...
    ) -> Result<u64, ()>
    {
        ...
        let tmp_arrays = self.tmp_arrays
            .iter()
            .map(|&(size, id)| ArrayArg(executor.allocate_array::<i8>(size), id.into()))
            .collect_vec();
