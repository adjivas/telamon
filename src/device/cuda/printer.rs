...
fn inst_arg(arg: &InstArg, namer: &mut NameMap) -> String {
    match *arg {
        ...
        VecInst(inst, dim, size) => {
            let names = (0..size)
                .map(|i| namer.indexed_inst_name(inst, dim, i).to_string())
                .collect_vec()
                .join(", ");
        ...
        VecOp(op, dim, size) => {
            let names = (0..size)
                .map(|i| namer.indexed_op_name(op, dim, i).to_string())
                .collect_vec()
                .join(", ");
            format!("{{{names}}}", names = names)
        }
...
fn cfg<'a>(fun: &Function, c: &Cfg<'a>, namer: &mut NameMap) -> String {
    match *c {
        ...
        Cfg::ParallelInductionLevel(ref level) => parallel_induction_level(level, namer),
    }
}
...
) -> String
{
    match dim.kind() {
        DimKind::LOOP => {
            ...
            let (var_init, var_step): (String, String) = ind_levels
                .map(|level| {
                    let t = ptx_type(level.t());
                    let dim_id = level.increment.map(|(dim, _)| dim);
                    let ind_var = namer.name_induction_var(level.ind_var, dim_id);
...
) -> String
{
    ...
    let (var, mut insts) = fun.block_dims()[1..].iter().fold(
        (d0, vec![]),
        |(old_var, mut insts), dim| {
...
pub fn function(function: &Function, gpu: &Gpu) -> String {
    ...
    let mut init = Vec::new();
    {
        ...
        mem_decls = function
            .mem_blocks()
            .flat_map(|block| match block.alloc_scheme() {
                AllocationScheme::Shared => Some(shared_mem_decl(block, name_map)),
...
pub fn host_function(fun: &Function, gpu: &Gpu, out: &mut Write) {
    ...
    let extern_param_names = fun.params
        .iter()
        .map(|x| &x.name as &str)
        .collect_vec()
        .join(", ");
    ...
    let params = fun.device_code_args()
        .map(|p| match *p {
