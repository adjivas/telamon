...
impl<T> cmp::Ord for ExplainedBound<T> {
    fn cmp(&self, other: &Self) -> cmp::Ordering { unwrap!(self.partial_cmp(other)) }
}
...
impl FastBound {
    ...
    /// * `iterations` - the number of times the computation bounded by `self` is
    /// repeated. * `max_par` -  the maximum number of independent jobs that can be
    /// built to compute   `iterations` times the computation bounded by self.
...
impl HwPressure {
    ...
    pub fn zero(device: &Device) -> Self {
        HwPressure::new(
            0f64,
            device.bottlenecks().iter().map(|_| 0f64).collect(),
        )
    }
    ...
    /// Adds the pressure of another computation, repeated in parallel. Ignores the
    /// latency.
