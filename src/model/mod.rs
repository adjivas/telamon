...
// is issued. For this, either double the nodes or subtract the size of buffers to the
// next  issue.
...
use model::hw_pressure::{BottleneckLevel, FastBound};
...
)
{
    ...
    for &pred in &predecessors {
        ...
        let iter_lat = cycle_lat
            .clone()
            .iterate(action.iterations - 2, level_id);
    ...
    for &(point, ref lat) in &levels[action.level_id].back_edges {
        ...
        for &pred in &predecessors {
            ...
            let latency = init_lat.clone().chain(
                point,
                lat.clone().iterate(action.iterations - 2, level_id),
            );
            ...
            if action.iterations >= 3 {
                ...
                let latency = init_lat
                    .chain(
                        point,
                        lat.clone().iterate(action.iterations - 3, level_id),
                    )
