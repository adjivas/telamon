...
//!
//! A level is akin to a loop, with a body and an iteration count, but it can represent
//! the intersection of the body of multiple iteration dimensions. This is necessary when
//! the nesting order of two loops of sizes N and M is left unspecified. Indeed, when
...
) -> HwPressure
{
    ...
    // Get the list of inner dimensions and inner dimensions on wich the pressure is
    // summed.
    let inner_dim_sets = dims.iter()
...
where
    IT: Iterator<Item = &'a VecSet<T>>,
    T: std::cmp::Ord + Clone + 'a,
{
    it.next().map(|out| {
...
/// with and without the loop.  We then build the minimal dag for the order defined such
/// as X < Y iff:
/// - nesting(X) < nesting(Y)
...
) -> (Vec<Level>, Vec<DimMap>)
{
    ...
    let dim_levels = dag.nodes()
        .iter()
        .enumerate()
        .flat_map(|(start_id, start)| {
            let nodes = dag.nodes();
        ...
        .flat_map(|dims| {
            // We only need to keep the sequential part of multi-dim levels as they are
            // only needed to iterate on the dimensions.
            if dims.len() <= 1 {
                Some(dims)
            } else {
                let sequential = dims.into_iter()
                    .filter(|&d| {
                        let kind = space.domain().get_dim_kind(d);
                        (kind & !DimKind::BLOCK)
                            .is(DimKind::SEQUENTIAL)
                            .is_true()
                    })
                    .collect::<VecSet<_>>();
    ...
    let levels = std::iter::once(VecSet::default())
        .chain(dim_levels)
        .unique();
    let levels = levels
...
impl LevelDag {
    ...
    fn gen_node_id(
        &mut self,
        local_info: &LocalInfo,
        level_dims: &[ir::dim::Id],
        dep_map_size: usize,
    ) -> usize
    {
        let before = level_dims
            .iter()
            .map(|&d| &local_info.nesting[&d.into()].before_self);
        let nodes = &mut self.nodes;
        *self.node_ids
            .entry(unwrap!(intersect_sets(before)))
            .or_insert_with(|| {
    ...
    )
    {
        for (node_dims, &node_id) in &self.node_ids {
            if dims <= node_dims {
                self.nodes[node_id]
                    .2
                    .add_dep(dep_start, dep_end, dep_lat.clone());
            }
        }
    }
    ...
    pub fn dependencies(&self, node: DagNodeId) -> &DependencyMap {
        &self.nodes[node.0].2
    }
    ...
    ) -> Vec<(DagNodeId, DagAction, Vec<DagNodeId>)>
    {
        ...
        for (dims, &from) in &self.node_ids {
            ...
            for repeat in self.nodes[from].0.drain(..) {
                ...
                let nodes_after = nodes_after
                    .iter()
                    .cloned()
                    .filter(|&(after_dims, _)| after_dims >= dims)
                    .map(|(_, &id)| DagNodeId(id))
                    .collect_vec();
                actions.push((
                    DagNodeId(from),
                    DagAction::Repeat(repeat),
                    nodes_after,
                ));
            }
