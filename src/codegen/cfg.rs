...
impl<'a> Cfg<'a> {
    ...
    pub fn dimensions(&self) -> impl Iterator<Item = &Dimension<'a>> {
        match *self {
            Cfg::Root(ref body) => Box::new(body.iter().flat_map(|cfg| cfg.dimensions()))
    ...
    pub fn induction_levels(&self) -> impl Iterator<Item = &InductionLevel> {
        match *self {
            ...
            Cfg::ParallelInductionLevel(ref level) => {
                Box::new(std::iter::once(level)) as _
            }
        }
    }

    /// Builds a CFG from a list of `CfgEvent`.
    fn from_events<IT>(events: &mut IT) -> Cfg<'a>
    where IT: Iterator<Item = CfgEvent<'a>> {
        ...
        loop {
            match events.next() {
                ...
                Some(Exit(_, ExitEvent::SeqDim(dim))) => return Cfg::Loop(dim, body),
                None => {
                    if let Some(&Cfg::Barrier) = body.last() {
                        body.pop();
                    }
...
impl<'a> fmt::Debug for Cfg<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            Cfg::Loop(ref dim, ref inners) => write!(
                f,
                "Loop([{:?}], {:?})",
                dim.dim_ids().format(","),
                inners
...
)
{
    ...
    block_dims.sort_by(|lhs, rhs| {
        ...
        match space
            .domain()
            .get_order(lhs.id().into(), rhs.id().into())
        {
    ...
    (
        block_dims,
        Itertools::flatten(thread_dims.into_iter()).collect(),
        events,
    )
}
