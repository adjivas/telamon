...
impl<'a, 'b> NameMap<'a, 'b> {
    ...
    pub fn name_inst_id(&self, id: InstId, dim_map: &DimMap) -> &str {
        ...
        for &(lhs, rhs) in dim_map.iter() {
            indexes
                .remove(&rhs)
                .map(|idx| indexes.insert(lhs, idx));
        }
        let (ref dims, ref name_array) = self.insts[&id];
        let idx_vals = dims.iter()
    ...
    fn decl_inst(&mut self, inst: &Instruction) {
        ...
        let names = (0..num_name)
            .map(|_| self.gen_name(inst.t()))
            .collect_vec();
        let array = NDArray::new(dim_sizes, names);
        assert!(
            self.insts
                .insert(inst.id(), (dim_ids, array))
                .is_none()
        );
    }
    ...
    fn decl_alias(&mut self, alias: &Instruction, base: InstId, dim_map: &DimMap) {
        ...
        let names = {
            ...
            let permutation = {
                let mut base_pos: HashMap<_, _> = base_dims
                    .iter()
                    .enumerate()
                    .map(|(i, x)| (*x, i))
                    .collect();
                for &(lhs, rhs) in dim_map.iter() {
                    base_pos
                        .remove(&lhs)
                        .map(|pos| base_pos.insert(rhs, pos));
                }
                dim_ids
                    .iter()
    ...
    ) -> Option<String>
    {
        ...
        match self.size_casts.entry((size, t)) {
            ...
            hash_map::Entry::Vacant(entry) => Some(
                entry
                    .insert(self.namer.borrow_mut().name(t))
                    .to_string(),
            ),
        }
    }
