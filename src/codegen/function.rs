...
use codegen::{cfg, dimension, Cfg, Dimension, InductionLevel, InductionVar};
use ir;
use ir::prelude::*;
...
impl<'a> Function<'a> {
    ...
    pub fn build(space: &'a SearchSpace<'a>) -> Function<'a> {
        ...
        let insts = space
            ...
            .collect_vec();
        let device_code_args = dims.iter()
            .flat_map(|d| d.host_values(space))
            .chain(
                induction_vars
                    .iter()
                    .flat_map(|v| v.host_values(space)),
            )
            .chain(insts.iter().flat_map(|i| i.host_values(space)))
            .chain(
                precomputed_indvar_levels
                    .iter()
        ...
        let device_code_args = device_code_args
            .into_iter()
...
impl<'a> InternalMemBlock<'a> {
    ...
    pub fn new(
        block: &'a ir::mem::InternalBlock<'a>,
    ...
    pub fn new(instruction: &'a ir::Instruction<'a>, space: &SearchSpace) -> Self {
        let instantiation_dims = instruction
            ...
            .map(|&dim| {
                (
                    dim,
                    unwrap!(space.ir_instance().dim(dim).size().as_int()),
                )
            })
            .collect();
        ...
        let t = unwrap!(
            space
                .ir_instance()
                .device()
                .lower_type(instruction.t(), space)
        );
        Instruction {
            instruction,
            instantiation_dims,
