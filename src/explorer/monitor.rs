//! This file exposes a single function, monitor, that is launched in a special thread
//! and pulls the evaluations results, store them and then updates the Store accordingly.
...
impl<T> From<TimeoutError<T>> for CommEnd {
    fn from(_: TimeoutError<T>) -> Self { CommEnd::TimedOut }
}
...
/// Depending on the value of the evaluation we just did, computes the new cut value for
/// the store Can be 0 if we decide to stop the search
...
/// Given a receiver channel, builds a future that returns whenever the tree has been
/// fully explored or the timeout has been reached
fn block_until_timeout<'a, 'b, T>(
    config: &'b Config,
    receiver: channel::mpsc::Receiver<MonitorMessage<'a, T>>,
    t0: Instant,
    candidate_store: &'b T,
...
/// All work that has to be done on reception of a message, meaning updating the best
/// cand if needed, logging, committing back to candidate_store
fn handle_message<'a, T>(
    config: &Config,
    ...
    T: Store<'a>,
{
    ...
    warn!(
        "Got a new evaluation, bound: {:.4e} score: {:.4e}, current best: {:.4e}",
        cand.bound.value(),
        eval,
        best_cand
            .as_ref()
            .map_or(std::f64::INFINITY, |best: &(Candidate, f64)| {
                best.1
            })
    );
    let change = best_cand
        .as_ref()
...
/// Builds and returns a timer that suits our needs - that is, which timeout can be set
/// to at least a few tens of hours
