...
/// Entry point of the exploration. This function returns the best candidate that it has
/// found in the given time (or at whatever point we decided to stop the search -
/// potentially after an exhaustive search)
pub fn find_best<'a, 'b>(
    config: &Config,
    context: &'b Context<'b>,
...
) -> Option<SearchSpace<'a>>
{
    match config.algorithm {
        config::SearchAlgorithm::MultiArmedBandit(ref band_config) => {
            let new_candidates = search_space
                .into_iter()
                .map(|space| {
...
/// Launch all threads needed for the search. wait for each one of them to finish.
/// Monitor is supposed to return the best candidate found
fn launch_search<'a, T>(
    config: &Config,
    candidate_store: T,
    context: &Context,
) -> Option<SearchSpace<'a>>
