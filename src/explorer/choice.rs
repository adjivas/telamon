...
pub fn list<'a>(space: &'a SearchSpace<'a>) -> impl Iterator<Item = Choice> + 'a {
    ...
    fun.layouts_to_lower()
        ...
        .chain(fun.dims().enumerate().flat_map(move |(i, lhs)| {
            // TODO(search_space): avoid picking ordering decisions that have little
            // impact. For this, we should avoid dimension-instruction and
            // dimension-vector dim orderings. The problem is that we do not
            // know wich choice to pick in the end.
            let lhs = lhs.bb_id();
